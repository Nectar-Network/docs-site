...
These are the **current** security-hardened (audit-prep) testnet contracts. See [Contract Addresses](../reference/contract-addresses) for the authoritative list and explorer links.
...
:::info
On testnet, USDC is **Circle's testnet USDC** — a Circle-issued Stellar Asset Contract (name "USD Coin", symbol "USDC", 7 decimals) from the [Circle faucet](https://faucet.circle.com). Mainnet will use **Circle's mainnet USDC** in Tranche 3, at which point you override `USDC_CONTRACT`, `NETWORK_PASSPHRASE`, and the RPC/Horizon endpoints. Phoenix has no published testnet deployment, so `PHOENIX_ROUTER` is normally left empty on testnet.
:::
